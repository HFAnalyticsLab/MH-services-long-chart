...
## Performance against 18 week target in Scottish CAMHS {.tabset}

- **Measure code:** TotalPatientsSeen and NumberOfPatientsSeen0To18Weeks
- **Measure description:** 
- **Source:** NHS Scotland [Open Data](https://www.opendata.nhs.scot/dataset/child-and-adolescent-mental-health-waiting-times)

### Raw time series

```{r scot.90, echo=TRUE, message=FALSE, warning=FALSE}
#Data
...
```{r scot.90.avg, echo=TRUE, message=FALSE, warning=FALSE}
...
## People still receiving treatment in Scottish CAMHS {.tabset}
...
```{r scot.cyp01, echo=TRUE, message=FALSE, warning=FALSE}
#Data
...
```{r scot.cyp01.avg, echo=TRUE, message=FALSE, warning=FALSE}
#Average per calendar year
CAMHS_Scotland_open %>%
  mutate(.,year=lubridate::year(year_month)) %>% 
...
## People waiting for treatment in Scottish CAMHS {.tabset}
...
```{r scot.cypwait, echo=TRUE, message=FALSE, warning=FALSE}
#Data
...
```{r scot.cypwait.avg, echo=TRUE, message=FALSE, warning=FALSE}
#Average per calendar year
CAMHS_Scotland_waiting %>%
  mutate(.,year=lubridate::year(year_month)) %>% 
  group_by(Metric,year) %>%
...
## New referrals to CYPMHS {.tabset}
...
```{r cyp32.raw.time.series, echo=TRUE, message=FALSE, warning=FALSE}
#Data
...
```{r cyp32.avg, echo=TRUE, message=FALSE, warning=FALSE}
#Average per calendar year
CAMHS_data_cyp32 %>%
  mutate(.,year=lubridate::year(start_date)) %>% 
  group_by(MEASURE_ID,MEASURE_KEY,year) %>%
...
```{r cyp32.avg.bis, echo=TRUE, message=FALSE, warning=FALSE}
#Average per calendar year
CAMHS_data_cyp32 %>%
  mutate(.,year=lubridate::year(start_date)) %>%
  filter(.,month_num>=4&month_num<=12) %>%
...
```{r ED.year.average, echo=TRUE, message=FALSE, warning=FALSE,fig.width=8,fig.height=6}
target_time_ed_data %>%
  arrange(.,end_date) %>% 
...
```{r ed.targets2, echo=TRUE, message=FALSE, warning=FALSE,fig.width=8,fig.height=6}
#Eating disorders data
...
```{r combined.ed, echo=FALSE, message=FALSE, warning=FALSE}

combined.ed.data <- plyr::rbind.fill(ref_ed_data_new,target_time_ed_data) %>%
  filter(.,end_date>ymd("2020-08-01")) %>% 
  ...
  arrange(.,Type,end_date)

#fwrite(combined.ed.data,paste0(onedrive_charts_data,"ED_charts.csv"))
```

```{r crisis.referrals, echo=TRUE, message=FALSE, warning=FALSE,fig.width=8,fig.height=6}
#Crisis referrals
...
```{r ED.referrals, echo=TRUE, message=FALSE, warning=FALSE,fig.width=8,fig.height=6}
#ED referrals
